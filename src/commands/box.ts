...
export = async function(data:commandSender){
    ...
    await data.message.channel.send(EmbedConstructor.playerBoxes(thisPlayer))
    const messageCollection = await data.message.channel.awaitMessages((m)=>m.author.id==userId,{"max":1})
    if (parseInt(messageCollection.first().content)){
        ...
        if (number>0 && number<=thisPlayer.box.length){
            ...
            data.message.channel.send(EmbedConstructor.boxOpened(weaponOpened,boxToOpen))
        }else{
